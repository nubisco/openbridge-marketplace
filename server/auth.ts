...
export async function verifyPlatformToken(token: string): Promise<AuthPayload | null> {
  ...
  const expectedApp = (process.env.PLATFORM_APP_ID ?? 'marketplace').trim()
  const claims = await _verifyPlatformJwt(token, issuer)
  if (!claims) return null

  if (claims.app !== expectedApp) return null

  const email = claims.email.toLowerCase().trim()
  const display =
    typeof claims.name === 'string' && claims.name.trim().length > 0 ? claims.name.trim() : email.split('@')[0]
